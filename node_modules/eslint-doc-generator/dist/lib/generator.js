...
function expectContent(ruleName, contents, content, expected) {
    ...
    if (hasContent !== expected) {
        ...
        expected ? '' : 'not '}have included: ${content}`);
        process.exitCode = 1;
    }
}
function expectSectionHeader(ruleName, contents, possibleHeaders, expected) {
    const found = possibleHeaders.some((header) => findSectionHeader(contents, header));
    if (found !== expected) {
        if (possibleHeaders.length > 1) {
            console.error(`\`${ruleName}\` rule doc should ${expected ? '' : 'not '}have included ${expected ? 'one' : 'any'} of these headers: ${possibleHeaders.join(', ')}`);
        }
        else {
            console.error(`\`${ruleName}\` rule doc should ${expected ? '' : 'not '}have included the header: ${possibleHeaders.join(', ')}`);
        }
        process.exitCode = 1;
    }
}
...
export async function generate(path, options) {
    ...
    for (const { name, description, schema } of details) {
        ...
        const newHeaderLines = generateRuleHeaderLines(description, name, plugin, configsToRules, pluginPrefix, path, pathRuleDoc, configEmojis, ignoreConfig, ruleDocNotices, ruleDocTitleFormat, urlConfigs, urlRuleDoc);
        const contents = readFileSync(pathToDoc).toString();
        const contentsNew = await postprocess(replaceOrCreateHeader(contents, newHeaderLines, END_RULE_HEADER_MARKER), resolve(pathToDoc));
        if (check) {
    ...
    for (const pathRuleListItem of Array.isArray(pathRuleList)
        ...
        const fileContentsNew = await postprocess(updateRulesList(details, fileContents, plugin, configsToRules, pluginPrefix, pathRuleDoc, pathToFile, path, configEmojis, ignoreConfig, ruleListColumns, ruleListSplit, urlConfigs, urlRuleDoc), resolve(pathToFile));
        if (check) {
